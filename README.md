...
En este juego, el objetivo del jugador es sumar lo más cerca posible a 21 sin pasarse. Cada participante comienza con dos cartas. El jugador decide si quiere otra carta (“pedir”) o si desea detenerse (“plantarse”). La banca (la máquina) juega después, siguiendo una lógica preestablecida (por ejemplo, pedir hasta llegar a 17). Al final se comparan los puntajes para determinar al ganador.
...
Este es un juego por turnos en el que el jugador debe pedirle a la IA una carta de un valor específico (por ejemplo: “¿Tienes algún 5?”). Si el oponente tiene una o más cartas de ese valor, las entrega. Si no, el jugador “pesca” del mazo. El objetivo es formar la mayor cantidad de pares posibles. El juego termina cuando el mazo se agota o no hay más cartas en manos.

**Elementos clave:**

- Comunicación por texto entre jugador y máquina  
...
El programa estará dividido en secciones lógicas mediante el uso de funciones. Se incluirán archivos separados si es necesario (por ejemplo, archivos `.h` o `.cpp` independientes), lo que permitirá una estructura más clara y profesional.
...
En primer lugar, extendemos nuestro reconocimiento y gratitud al docente Guillermo Cortés, quien durante todo el curso de Fundamentos de Programación nos brindó una enseñanza clara, paciente y comprometida.
